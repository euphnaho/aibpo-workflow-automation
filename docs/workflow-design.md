...
以下の番号は §8 のスケジュール表、および README.md のチェックリストと共通で使う。

| # | マイルストーン |
|---|---|
| ① | 既存の `初回商談一覧` 生成スクリプトを調査する |
| ② | `初回商談一覧` に紐付け用列を追加し、`案件化中商談一覧` / `失注リスト` を新規作成する |
| ③ | ①の調査結果をふまえ、カレンダーID列を既存処理に組み込む |
| ④ | Slack Bot を作成し `chat:write` スコープを付与、対象チャンネルに招待する |
| ⑤ | Slack Workflow Builder で「商談結果報告」を設定する（§5） |
| ⑥ | `gas/` のコードを配置・スクリプトプロパティ設定・Web Appデプロイする（§6） |
| ⑦ | `notifyCompletedFirstMeetings` に時間主導トリガーを設定する |
| ⑧ | ダミーデータでE2Eテストする |
| ⑨ | チームへ共有・運用開始を案内する |

## 8. スケジュール（8月末までの進め方）

2〜3時間/日の稼働を想定。①既存スクリプト調査と④Slack Bot作成申請は承認待ちが発生しうるため最優先で着手する。

| 日程 | 該当マイルストーン | 内容 | 目安時間 |
|---|---|---|---|
| 8/18(火) | ①④ | 既存スクリプト調査＋Slack Bot作成申請 | 2〜3h |
| 8/19(水) | ② | 初回商談一覧に列追加／新規シート2枚作成 | 1〜2h |
| 8/20(木) | ③ | カレンダーID列を既存処理へ組み込み | 1〜2h |
| 8/21(金) | ⑤ | Slack Workflow Builder設定 開始 | 2〜3h |
| 8/22-23(土日) | — | 予備（⑤が押した場合の吸収） | — |
| 8/24(月) | ⑤ | Slack Workflow Builder設定 完了（3分岐＋Webリクエストステップ） | 2h |
| 8/25(火) | ⑥⑦ | GASデプロイ・スクリプトプロパティ設定＋トリガー設定 | 1.5〜2h |
| 8/26(水) | ⑧ | E2Eテスト（案件化ルート） | 2h |
| 8/27(木) | ⑧ | E2Eテスト（検討中・失注ルート）＋デバッグ | 2〜3h |
| 8/28(金) | ⑧ | 不具合修正・再テスト | 2h |
| 8/29-30(土日) | — | バッファ（テスト漏れ・想定外の不具合対応） | — |
| 8/31(月) | ⑨ | チーム共有・運用開始案内 | 0.5〜1h |
