...
最終更新: 2026-08-17（v7: 追いかけ中商談リスト・失注リストの作成先をKPI管理シート側に変更）
...
### 5.4 `追いかけ中商談リスト`（KPI管理シート・新規シート）

`初回商談一覧`と同じスプレッドシート（このApps Scriptプロジェクトが紐づくファイル）に作成する
（当初は案件管理シート側の予定だったが、実装時にKPI管理シート側に変更した）。
...
### 5.5 `失注リスト`（KPI管理シート・新規シート）

`追いかけ中商談リスト`と同様、`初回商談一覧`と同じスプレッドシートに作成する。
...
- `Config.gs` : スクリプトプロパティ（Incoming Webhook URL、案件管理シート（`案件リスト`が
  入っている方）のスプレッドシートID等）の読み書きヘルパー。`追いかけ中商談リスト`・`失注リスト`は
  KPI管理シート側（このプロジェクトが紐づくファイル）にあるため、別途IDの管理は不要。
...
- `NotifySlack.gs` : `notifyCompletedFirstMeetings()`（商談終了検知→初回通知）と
  `renotifyDueFollowUps()`（NA日到来→再通知、ステップ6に対応。`追いかけ中商談リスト`は
  KPI管理シート側にあるため `DEAL_SHEET_ID` は使わない）。Incoming Webhookで送信する。
- `ReconcileDealSheets.gs` : `reconcileDealSheetWrites()`。想定オペレーション ステップ5〜6（反映側）に対応。
  `案件リスト`（案件管理シート・`DEAL_SHEET_ID`で開く） / `追いかけ中商談リスト` / `失注リスト`
  （どちらもKPI管理シート側）を巡回し、`初回商談一覧`の未回答行と企業名の完全一致（無ければ
  商談実施日が同じで候補1件のみの場合に限り仮マッチ）で突き合わせて、企業名・担当者名の確定値と
  案件化有無を更新する。案件化・失注が確定した場合は `追いかけ中商談リスト` 側の該当行もクローズする。
  マッチ時に `突合` シートの `企業名(確定)` 列（存在する場合のみ）も同じ値で更新する。
- `SheetHelpers.gs` : シート操作の共通関数（行検索・追記・更新・upsert）。

### デプロイ手順（概要）

1. KPI管理シートに紐づく Apps Script プロジェクトに `gas/` 配下のファイルを追加。
   （`案件管理シート`（`案件リスト`が入っている方）は別ファイルなので
   `SpreadsheetApp.openById(DEAL_SHEET_ID)` で開く。`追いかけ中商談リスト`・`失注リスト`は
   KPI管理シート側にあるため `SpreadsheetApp.getActiveSpreadsheet()` で参照する）
2. スクリプトプロパティに以下を設定:
