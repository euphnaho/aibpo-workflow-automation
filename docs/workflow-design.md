...
最終更新: 2026-08-17（v4: Incoming Webhook方式に統一、完了報告はSlackワークフロー側で実施）
...
flowchart TD
    ...
    D --> E{"商談終了時刻を\n過ぎたか？\n(GAS 時間主導トリガー)"}
    E -- Yes・未通知 --> F["Slack通知\n3ボタン: 案件化した/進行中/失注した"]
    F -- 案件化した --> G1["既存の案件化Slackワークフロー\n(案件リストへの書き込みは既存WF側が担当)"]
    F -- 進行中(未失注) --> G2["新規ワークフロー\nNA日を回収"]
    F -- 失注した --> G3["新規ワークフロー\n失注理由を回収"]
    G1 --> I["商談DB更新用のWebリクエストのみ\n(案件リストは既存WFが書き込み済み)"]
    G2 --> J["追いかけ中商談リストへupsert"]
    G3 --> K[失注リストへ追記]
    I --> L["初回商談一覧を更新\n(案件化有無/企業名確定)"]
    J --> L
    K --> L
...
1. **ボタンの時点で3方向に分かれる**こと。案件化のケースは既存の案件化Slackワークフローへそのまま進み、
   新規に作るのは「進行中（未失注）」「失注した」の2ワークフローだけでよい。
2. **未案件化の商談がループとして「4. Slack通知」に戻ってくる**こと。
...
GAS からは自動生成できないため、Slack 側で以下を手動設定する。Slack通知のボタンは3つあり、
それぞれ別のワークフロー（うち1つは既存）へ飛ぶ。「案件化ステータスを質問する」ステップは
不要になった。どのボタンが押されたかで、遷移先のワークフロー自体が結果を表している。

### 6.1 既存の「案件化」ワークフローへの追加設定

案件リストへの書き込みは既存ワークフローがすでに行っているため、そこは変更しない。
商談DB（初回商談一覧）を更新するためだけに、末尾へ1ステップ追加する。

1. 既存ワークフローのリンクトリガーに、入力変数 `calendar_id` を追加する
   （`company_name` 等の変数がすでにあれば流用し、無ければ追加する）。
2. GASが投稿するSlackメッセージの「案件化した」ボタンURLに、
   `calendar_id` / `company_name` / `contact_name` / `sales_rep` をクエリパラメータとして付与する
   （§7 `EXISTING_DEAL_WORKFLOW_URL` 参照）。
3. ワークフローの最後に **「Webリクエストを送信」** ステップを追加し、GAS Web App の `/exec` URLへ
   `{ secret, calendar_id, company_name, contact_name, sales_rep, status: "案件化した" }` をPOSTする
   （`company_name` は既存ワークフロー内で営業担当が確定させた値を使う）。
4. その後に **「メッセージを送信」** ステップを追加し、チャンネルへ完了報告を投稿する
   （例: 「✅ {{company_name}} を案件化として記録しました」）。GASからは完了報告を行わない。

### 6.2 新規ワークフロー（進行中／失注）

「進行中（未失注）」「失注した」それぞれに独立したシンプルなワークフローを作る
...
3. 「Webリクエストを送信」: `{ secret, calendar_id, company_name, contact_name,
   sales_rep, status: "進行中（未失注）", next_action, na_date, probability }` をPOST。
4. 「メッセージを送信」で完了報告（例: 「📝 {{company_name}} を追いかけ中商談リストに記録しました」）。

**失注した用**
1. リンクトリガーの入力変数: `calendar_id`, `company_name`, `contact_name`, `sales_rep`。
2. フォーム: 企業名・担当者名、失注区分、失注理由（マスター参照の選択肢）、詳細メモ。
3. 「Webリクエストを送信」: `{ secret, calendar_id, company_name, contact_name,
   sales_rep, status: "失注した", lost_type, lost_reason, lost_reason_detail }` をPOST。
4. 「メッセージを送信」で完了報告（例: 「📕 {{company_name}} を失注リストに記録しました」）。

いずれもURL・Method・Bodyの形式は6.1と同じ（認証シークレットはヘッダーではなくJSONボディに含める）。
完了報告はどのワークフローも自身の「メッセージを送信」ステップで行い、GASからは投稿しない。

## 7. GAS 実装

`../gas/` 配下に実装を置く。詳細は各ファイルのコメント参照。

- `Config.gs` : スクリプトプロパティ（Incoming Webhook URL、案件管理シートのスプレッドシートID、
  共有シークレット等）の読み書きヘルパー。
...
- `WebhookHandler.gs` : Web App のエントリポイント `doPost(e)`。
  「案件化した」は既存ワークフローから呼ばれ、商談DBの更新のみ行う（案件リストへは書き込まない）。
  「進行中（未失注）」「失注した」はそれぞれ `追いかけ中商談リスト` / `失注リスト` へ反映する。
  いずれも最後に `初回商談一覧` の企業名・担当者名を確定値で更新する。完了報告は行わない
  （各Slackワークフロー側の「メッセージを送信」ステップが担当。§6参照）。
...
3. `doPost` を含むプロジェクトを「ウェブアプリ」としてデプロイし、発行された `/exec` URL を
   3つのワークフロー（既存の案件化ワークフロー含む）それぞれの「Webリクエストを送信」ステップに設定する。
4. 3つの時間主導トリガーを設定する。
...
| ⑤ | Slack Workflow Builderを設定する：既存の案件化ワークフローへWebリクエストステップを追加＋新規「進行中」「失注」ワークフローを作成 | 2〜3h |
...
- **既存の案件化ワークフローの管理者との調整**: `calendar_id` 入力変数の追加と「Webリクエストを送信」
  ステップの追加を、既存ワークフローの編集権限を持つ人に依頼する必要がある。また既存ワークフロー内で
  企業名を入力/確定しているフィールド名を確認し、GASへ送る `company_name` にどの値を使うか合わせる。
